...
elseif ($allowedRecordNames.Count -gt 0 -and ($allowedRecordNames -notcontains $name.ToLowerInvariant())) {
    ...
    # Validate reqIP is specifically an IPv4 address for A records.
    [IPAddress]$parsedReqIP = $null
    $isParsableIP = [System.Net.IPAddress]::TryParse($reqIP, [ref]$parsedReqIP)
    $isValidReqIP = $isParsableIP -and ($parsedReqIP.AddressFamily -eq [AddressFamily]::InterNetwork)
    ...
    if (-not $isValidReqIP) {
        ...
        try {
            $currentRecord = Get-AzDnsRecordSet -Name $name -RecordType A -ZoneName $zone -ResourceGroupName $resourceGroupName -ErrorAction Stop
        }
        catch {
